...
//  const char kMainFunctionName[] = "module.simple_mul";
  ...
  // Initial buffer contents single image input
  static  const float kFloat4[1][224][224][3];
  ...
  for (iree_host_size_t i = 0; i < IREE_ARRAYSIZE(results); ++i) {
    if ((results[i] > 0.01) || (results[i] < -0.01)) {
      printf("index: %d, probability: %g\n", i, results[i]);
    }
    /*
    if (results[i] != 8.0f) {
